...
var root = 'https://api.oneskyapp.com/2/';
...
module.exports = function(publicKey, privateKey) {
  ...
  var responseHandler = function(cb) {
    return function(err, res, body) {
      ...
      try {
        ...
        err = 'Could not parse response from OneSky: ' + body;
        return cb(err, null);
      }
  ...
  var post = function(path, data, cb) { request.get(createRequestOptions(path, data), responseHandler(cb)); };
  ...
  methods.project.platforms = function(project, cb) {
    get('projecst/platforms', { project: project }, cb);
  };
  ...
  methods.sso.getLink = function(uniqueId, name, cb) {
    var data = {
      'unique-id': 'bp+translator@brandonpaton.coma',
      'name': 'SSO translator BPa'
    }
