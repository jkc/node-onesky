...
module.exports = function(publicKey, privateKey) {
  ...
  var responseHandler = function(cb, parseResponse) {
    // default behavior is to parse the body as JSON
    parseResponse = typeof parseResponse !== 'undefined' ? !!parseResponse : true;
    return function(err, res, body) {
      ...
      try {
        if (typeof body !== 'object') {
          // attempt to parse, regardless of parseResponse, in case an error has been returned
          var parsedBody = JSON.parse(body);
        }
        else {
          // if body is already an object set parsedBody to it for consistency below
          parsedBody = body;
        }
      } catch(e) {
        if (parseResponse) {
          // parsing failed, but was expected to work
      ...
      // Catch onesky errors
      if (parsedBody && (parsedBody.error || parsedBody.response === 'error')) {
        err = 'Error from OneSky';
        if (parsedBody.error) err += ': ' + parsedBody.error;
        return cb(err, null)
      }

      // Return OneSky response
      cb(null, parseResponse ? parsedBody : body);
    };
  };

  // Helpers
  var get  = function(path, data, cb, parseResponse) { request(createRequestOptions('GET', path, data), responseHandler(cb, parseResponse)); };
  var post = function(path, data, cb, parseResponse) { request(createRequestOptions('POST', path, data), responseHandler(cb, parseResponse)); };
  var upload = function(path, data, cb, parseResponse) {
    var r = request(createRequestOptions('POST', path, null), responseHandler(cb, parseResponse)),
        form = r.form();
  ...
  methods.string.download = function (platform, locale, tag, format, cb) {
    // Parse platform
    if (typeof platform === 'string' || typeof platform === 'number') platform = { 'platform-id': platform }
    else if (platform.platformId) {
      platform['platform-id'] = platform.platformId;
    ...
    // Since we're expecting a file back, pass false as final argument to prevent parsing the response
    get('string/download', data, cb, false);
  };
  ...
  return {
    ...
    string: {
      ...
      download:     methods.string.download
    },
